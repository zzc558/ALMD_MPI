...
def train_model_energy_gradient(i=0, outdir=None, mode='training'):
    ...
    dir_idx = os.path.join(outdir, "index")
    ...
    with open(os.path.join(dir_idx, "train_val_idx_" + 'v%i' % i + ".npy"), 'wb') as fh:
        np.save(fh, i_train)
        np.save(fh, i_val)
    ...
    if masked_loss:
        ...
        mae_energy.set_scale(scaler.energy_std)
        mae_force.set_scale(scaler.gradient_std)
        train_metrics = {'energy': [mae_energy, lr_metric, r2_metric],
    ...
    try:
        outname = os.path.join(dir_save, "history_" + 'v%i' % i + ".json")
    ...
    try:
        ...
        print("Info: Plot fitting history...")
        ...
        print("Error: Could not plot fitting history")
